...
// Copyright ® 1999-2026
//-------------------------------------------------------
console.log( 'jestAlert: js/apps/jest/components/editing/tools/JestFileViewEditorPainterJani.js loaded' );

//-------------------------
// JestFileViewEditorPainterJani Class
//-------------------------
// This class enhances the file editor painter view with jani creation specific capabilities.
class JestFileViewEditorPainterJani extends JestFileViewEditorPainter {
	// Object propert(ies)
	...
	build ( name='file-view-editor-painter-jani', classes=[] ) {
		// Construct the parent panel.
		if ( classes===null ) classes = [];
		const defaultClasses = [ 'file-view-editor-painter-jani' ];
		super.build( name, defaultClasses.mergeUnique(classes) );
		// Initialize the view
		// Copy jani canvas content to this canvas.
	}
	...
	setFile( file ) {
		// Validate argument(s)
		if ( !(file instanceof JestFileJani) ) {
			console.warn( `Argument "file" must be of type JestJani.` );
			return false; // failed
		}
	...
	initView() {
		...
		this.anchor.resize( config.janiFullSpan, config.janiFullSpan );
	}
	...
	// Get jani rendering canvas.
	// RETURNS: [void].
	getViewRender() {
		// Return the jani bitmap canvas.
		const canvas	 = this.file.context.canvas;
		return canvas; // jani bitmap canvas
	}
}
