...
class AnimationAnimation extends AnimationObject {
	...
	layerCount	= 0;			// [int] Count of total number of layer(s).
	// ID Counter(s)
	...
	constructor( name ) {
		...
		this.createFrame();		// creates exactly 1 frame & 1 layer
	}
	...
	_clampAllViews() {
		...
		for ( const name in this.views ) {
			...
			const current = view.getLayerIndex();
			const max	= this.layerCount - 1;
			if ( current>max )
				view.setLayerIndex( max );
			// Clamp cursor selection(s).
			view.clampSelected();
		}
	}
	...
	//-------------------------
	// Inserts a new layer across ALL frames at a given index.
	// RETURNS: [void].
	// * index – [int|null] index to insert at, or [null] to append
	createLayerAt( index=null ) {
		//--------------------------------
		// Determine Target Index
		//--------------------------------
		// Clamp index.
		const frame0	= this.frames[0];
		const max		= frame0?.layers.length ?? 0;
		if ( index===null ) index = max;
		else index		= Math.max( 0, Math.min(index, max) );

		//--------------------------------
		// Insert New Layer in Each Frame
		//--------------------------------
		// Iterate all frames & add layer @ index.
		for ( const frame of this.frames )
			frame._createLayerAt( index ); // reindexes automatically

		//--------------------------------
		// Update Layer Count & Clamp Views
		//--------------------------------
		// Recount layers.
		this.layerCount = this.frames[0]?.layers.length ?? 0;
		this._clampAllViews(); // clamp
	}

	// Removes a layer from ALL frames at a specific index.
	// RETURNS: [boolean] true if removed
	// * index – [int] index to remove
	removeLayerAt( index ) {
		// Valdiate layer index.
		if ( this.frames.length===0 || index<0 || index>=this.layerCount )
			return false; // invalid index

		//--------------------------------
		// Remove Layer from All Frames
		//--------------------------------
		// Iterate all frames & remove layer.
		for ( const frame of this.frames )
			frame._removeLayerAt( index );
		...
		// Recount layers.
		this.layerCount = this.frames[0]?.layers.length ?? 0;
		this._synchLayerCount();	// Ensure all frame(s) contain same layer count.
	...
	clearLayers() {
		...
		//--------------------------------
		this.layerCount	= 0;		// none
		this._synchLayerCount();	// Ensure all frame(s) contain same layer count.
		this._clampAllViews();		// Clamp selection.
		return true;				// success
	}

	// Moves a layer from one index to another across every frame.
	// RETURNS: [void].
	// * fromIndex - [int] index of the layer to move.
	// * toIndex   - [int] index to move the layer to.
	moveLayerTo( fromIndex, toIndex ) {
		//--------------------------------
		// Validate
		//--------------------------------
		// Ignore if indexes are the same or out of bounds.
		if ( fromIndex===toIndex ) return;
		if ( fromIndex<0 || fromIndex>=this.layerCount ) return;
		if ( toIndex<0   || toIndex>=this.layerCount ) return;

		//--------------------------------
		// Apply Reorder to Every Frame
		//--------------------------------
		for ( const frame of this.frames ) {
			const layers = frame.layers;

			// Skip if layer array is malformed.
			if ( !Array.isArray(layers) || layers.length<=Math.max(fromIndex,toIndex) )
				continue;
		...
		//--------------------------------
		// Clamp Views + Optional Sorting
		//--------------------------------
		this._clampAllViews(); // update all views with new indexes

		// No need to recalc layerCount — it hasn’t changed
	}
	...
	_synchLayerCount() {
		...
		//--------------------------------
		// Ensure layer count is at least 1.
		if ( this.layerCount<1 ) {
			console.warn( '[AnimationAnimation] No layers remaining. Auto-created fallback layer at index 0.' );
			this.layerCount	= 1; // at least one layer required
		}

		//--------------------------------
		// Iterate All Frame(s) & Synch Layer Count
		//--------------------------------
		// Iterate all frames & synch layer count.
		for ( const frame of this.frames ) {
			if ( frame.layers.length<this.layerCount ) {
				// Create the difference of missing layers.
				const missingLayersCount = this.layerCount - frame.layers.length;
				for ( let i=0; i<missingLayersCount; i++ )
					frame._createLayer(); // create new layer
			}
		}
	}
	...
	_insertFrameAt( frame, index=null ) {
		...
		// -------------------------
		// Move Layer If Already In Animation
		// -------------------------
		...
		//-------------------------
		// Ensure layer count is synched.
		const expected = this.layerCount ?? 0;
		while ( frame.layers.length < expected )
			frame._createLayerAt( index );
	...
	// -------------------------
	// Remove a Frame (with View Clamp)
	// -------------------------
